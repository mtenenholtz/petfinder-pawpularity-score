...
if args.param_search:
    ...
    for k in range(1, 50):
        ...
        for i in range(5):
            ...
            knn = KNeighborsRegressor(n_neighbors=k).fit(
                train_df[emb_cols].values.astype(np.float32), 
                train_df['Pawpularity'].values.astype(np.float32)
            )
            preds = knn.predict(val_df[emb_cols].values.astype(np.float32))
    ...
    plt.plot(rmses)
    plt.savefig('data/plots/knn_rmses.png')